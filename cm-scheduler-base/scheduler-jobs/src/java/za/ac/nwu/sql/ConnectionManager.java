...
public class ConnectionManager {
    ...
    public Connection getConsolidationConnection() {
        Connection connection = null;
        if ("jdbc".equalsIgnoreCase(serverConfigurationService.getString("nwu.cm.kons.db", "jdbc"))) {
        ...
        if (connection == null) {
            LOG.error("ConnectionManager: No Consolidation JNDI/JDBC connection could be found");
        }
        return connection;
    }

    private Connection getConsolidationJDBCConnection() {
        Connection conn = null;
        try {
            Class.forName(serverConfigurationService.getString("nwu.cm.kons.jdbc.driver",
                "com.microsoft.sqlserver.jdbc.SQLServerDriver"));
            String url = serverConfigurationService.getString("nwu.cm.kons.jdbc.url",
                "jdbc:sqlserver://db-win1.puk.ac.za:1433;databaseName=KONSOLIDASIE");
            conn = DriverManager.getConnection(url,
                serverConfigurationService.getString("nwu.cm.kons.jdbc.user", "itbcjjvv"),
                serverConfigurationService.getString("nwu.cm.kons.jdbc.password", "kasteel123"));
        }
        catch (Exception e) {
            LOG.error("JDBC Consolidation Connection could not be established.", e);
        }
        return conn;
    }

    private Connection getConsolidationJNDIConnection() {
        Connection conn = null;
        try {
            Context ctx = new InitialContext();
            DataSource ds = (DataSource) ctx.lookup(serverConfigurationService
                    .getString("nwu.cm.kons.jndi.name"));
            conn = ds.getConnection();
        }
        catch (Exception e) {
            LOG.error("JDNI Consolidation Connection could not be established.", e);
        }
        return conn;
    }
}
