...
//            sql.append("group by m.course_code, m.course_level, m.course_module, c.campus_code, l.lecturer_id, m.method_of_del, m.present_cat");
            ...
            while (rset.next()) {
                ...
                Module module = new Module(rset.getString(1), rset.getString(2), rset.getString(3),
                        campus, year);
//                Module module = new Module(rset.getString(1), rset.getString(2), rset.getString(3),
//                        campus, year, rset.getInt(4), rset.getInt(5));
                if (withRelatedInfo) {
                    for (Module existingModule : modules) {
                        if (existingModule.equals(module)) {
                            existingModule.addLinkedLecturer(getLecturer(rset.getInt(5)));
                            existingModuleFound = true;
                            break;
                        }
                    }
                    if (!existingModuleFound) {
                        module.addLinkedLecturer(getLecturer(rset.getInt(5)));
    ...
    private Set<Student> getActiveStudents(final Module module) {
        ...
        Connection connection = connectionManager.getConsolidationConnection();
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        StringBuilder sql = new StringBuilder();
        try {
            ...
            pstmt = connection.prepareStatement(sql.toString());
            pstmt.setInt(1, module.getYear());
            pstmt.setString(2, module.getCanonicalCourseReference());
            pstmt.setInt(3, Integer.parseInt(module.getCampus().getNumber()));
            rset = pstmt.executeQuery();
            while (rset.next()) {
                students.add(new Student(rset.getString(1)));
            }
        }
        catch (SQLException e) {
            LOG.error("SQL error occured when performing DataManager.getStudents", e);
        }
        finally {
            try {
                connection.close();
    ...
    //    /**
    //     * Returns a boolean to indicate whether the lecturer is linked to a non deleted CourseSet. (in any year)
    //     */
    //    public boolean isLecturerLinkedToNonDeletedCourseSet(final Lecturer lecturer,
    //            final Module module) {
    //        LOG.info("Performing isLecturerLinkedToNonDeletedCourseSet (lecturer="
    //                + lecturer.toString()
    //                + ", module="
    ...
    //            sql.append("from CM_MODULES m, CM_LECTURER l ");
    ...
    //            pstmt.setString(5, lecturer.getUserName());
    //            rset = pstmt.executeQuery();
    //            while (rset.next()) {
    //                linked = rset.getInt(1) > 0;
    //            }
    //        }
    //        catch (SQLException e) {
    //            LOG.error(
    //                "SQL error occured when performing DataManager.isLecturerLinkedToNonDeletedCourseSet",
    //                e);
    //        }
    //        finally {
    //            try {
	...
	public List<String> getUsers(String subject, String campus, String year)
		...
		Connection con = connectionManager.getConsolidationConnection();
		try {

			PreparedStatement s = con
					.prepareStatement("select nommer from STUDENTINSKRYWING  where jaar = ? "
							+ "and vakkode = ? and kampuskode = ? and aktief = 'A'");
			s.setString(1, year);
			s.setString(2, subject);
			s.setInt(3, Integer.parseInt(campus));
