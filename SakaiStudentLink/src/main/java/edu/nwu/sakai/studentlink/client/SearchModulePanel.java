...
public class SearchModulePanel extends Composite {
    ...
    private CellTable<ModuleDetail> moduleResults = new CellTable<ModuleDetail>();
    ...
    public SearchModulePanel() {
        ...
        methodOfDeliveryListBox.setEnabled(false); // added due to unresolved changes on studenteinskrywing, waiting for NWU's decision  
        ...
        presentationCategoryListBox.setEnabled(false); // added due to unresolved changes on studenteinskrywing, waiting for NWU's decision  
    ...
    public void searchModules() {
        ...
        AsyncCallback<List<ModuleDetail>> callback = new AsyncCallback<List<ModuleDetail>>() {
            ...
            public void onSuccess(List<ModuleDetail> result) {
    ...
    public HashMap<SearchCriteria, String> buildModuleSearchCriteria() {
        ...
        //Mask allows "*" or "%" as wildcards. But Java searches (in sql) allows only the "%" symbol as wildcard.
        criteria.put(SearchCriteria.COURSE_CODE, moduleValue.substring(0, 4).replace("*", "%"));
        criteria.put(SearchCriteria.COURSE_LEVEL, moduleValue.substring(5, 6).replace("*", "%"));
        criteria.put(SearchCriteria.COURSE_MODULE, moduleValue.substring(7).replace("*", "%"));
    ...
    public void populateModuleResults(List<ModuleDetail> result) {
        moduleResults.setRowCount(result.size(), true);
        ListDataProvider<ModuleDetail> listViewAdapter = new ListDataProvider<ModuleDetail>();
	...
	private void populateMethodOfDeliveryList(List<ModuleDetail> result) {		
		...
		for (ModuleDetail moduleDetail : result) {   
			uniqueResultMap.put(moduleDetail.getMethodOfDeliveryCode().toString(), moduleDetail.getMethodOfDeliveryName());
		}
    ...
    private void populatePresentationCategoryList(List<ModuleDetail> result) {	
		...
		Map<String, String> uniqueResultMap = new HashMap<String, String>();
		for (ModuleDetail moduleDetail : result) {   
			uniqueResultMap.put(moduleDetail.getPresentationCategoryCode().toString(), moduleDetail.getPresentationCategoryName());
		}
		for (Map.Entry<String, String> entry : uniqueResultMap.entrySet()){
	...
	public String buildModuleDescr(ModuleDetail module) {
        StringBuilder strBuild = new StringBuilder(module.getCourseCode());
        strBuild.append(" ").append(module.getCourseLevel()).append(module.getCourseModule());
        String year = ((TextBox) searchLayout.getWidget(2, 1)).getText();
        strBuild.append(" ").append(module.getModulePresentingEngCampusName().substring(0, 1))
                .append(" ").append(year);
    ...
    public void clearPanel() {
        ...
        moduleResults.setRowData(0, new ArrayList<ModuleDetail>());
